  ...
  return (
    <div className="order-summary">
      ...
      <div className={`order-summary__content ${expanded ? 'order-summary__content--open' : ''}`}>
        ...
        {/* Line Item Editor (max 1) — subscription upgrade */}
        {subItem && (
          <CheckoutExtensionBlock type="Line Item Editor">
            <div className="extension-line-editor">
              <img src={subItem.image} alt={subItem.title} className="extension-line-editor__image" />
              <div className="extension-line-editor__info">
                <p className="extension-line-editor__title">{subItem.title}</p>
                <div className="extension-line-editor__prices">
                  <span className="extension-line-editor__sale">${(parseFloat(subItem.price) * 0.85).toFixed(2)}</span>
                  <span className="extension-line-editor__compare">${subItem.price}</span>
                </div>
                <p className="extension-line-editor__switch">Subscribe & Save 15%</p>
              </div>
              <select className="extension-line-editor__select">
                <option>Deliver Every</option>
                <option>2 weeks</option>
                <option>4 weeks</option>
                <option>6 weeks</option>
              </select>
            </div>
          </CheckoutExtensionBlock>
        )}
